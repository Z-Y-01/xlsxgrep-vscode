...
export class SearchWebviewProvider implements vscode.WebviewViewProvider {
    ...
    constructor(
        ...
        private readonly _onSearch: (data: ISearchData) => void
    ) {}
    ...
    private _processSearchInput(data: any) {
        ...
        if (targetVal.length <= 0) {
            vscode.window.showInformationMessage(
                `Please check if your input is empty.`
            );
            return;
        }
        ...
        if (data.bUseRegex) {
            try {
                ...
                vscode.window.showInformationMessage(
                    `Please check if the regular expression for your search content is entered correctly.`
                );
                return;
            }
        }
